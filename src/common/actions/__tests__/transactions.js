...
import { Wallet as WalletUtils, Transaction as TransactionUtil } from '../../utils'
...
describe('TransactionsActions', () => {
  ...
  it('`sendEther` should break if the destination address or the value are invalid', async function() {
  ...
  it.skip('`sendEther` should send ether to some address when there are funds avaible', async function() {
  ...
  it('`sendTransaction` should break if the wallet or the transaction is invalid', async function() {
    const mnemonics = WalletUtils.generateMnemonics()
    const wallet = WalletUtils.loadWalletFromMnemonics(mnemonics)
  ...
  it.skip('`sendTransaction` should send ether to some address when there are funds available', async function() {
    ...
    let txn = TransactionUtil.createTransaction(DESTINATION_ADDRESS, value)
